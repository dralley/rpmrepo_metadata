...
| `uservisible` | Whether the group is shown in UI listings (boolean). Libsolv defaults to `true` if not present. Groups with `<uservisible>false</uservisible>` are hidden unless otherwise overridden. |
| `biarchonly` | If `true`, only biarch packages from this group are installed on multilib systems (boolean). |
| `langonly` / `lang_only` | Language code — restricts the group to systems configured for that language. Both element names are accepted by libsolv, but libcomps only supports `langonly`. Rarely used in practice — primarily found in RHEL 6 comps? |
...
filtering (parsed by libcomps, not in the DTD). This is rarely used in practice.
...
| Type | Behavior |
|------|----------|
| `mandatory` | Always installed when the group is installed. Group install fails if a mandatory package has broken dependencies (but if missing, it will be ignored). |
...
"Servers", "System", "Desktops") in tools like Anaconda's package selection
screen.
...
### How libsolv represents environment categories
...
Langpacks are not part of the dependency resolution process.

libcomps parses the `<langpacks>` section and stores the mappings in a
dictionary. In DNF4, langpacks functionality was provided by the
`dnf-langpacks` plugin, which would read this data from libcomps and
automatically add language-specific weak dependencies to the transaction based
on the system's locale configuration.

libsolv does not parse or process langpacks at all. DNF5 uses libsolv
exclusively for loading comps data and does not link against libcomps, so
**DNF5 has no langpacks support**. The `<langpacks>` section is ignored
entirely — there is no mechanism to automatically install language packs based
on system locale.

A dependency resolver implementation can safely ignore langpacks entirely.
...
AppStream each carry their own comps.xml).

### Why multiple comps files exist

**Split-repository architectures** (RHEL/CentOS BaseOS + AppStream, Fedora
Everything + Modular) use separate comps.xml files to partition metadata by
repository purpose:

- **RHEL 9 BaseOS** (34 groups, 3 environments): Core OS packages, minimal
  installs, hardware enablement. Contains foundational groups like `core` (80
  packages: bash, dnf, NetworkManager), `hardware-support`, `minimal-environment`.
- **RHEL 9 AppStream** (63 groups, 4 environments): Applications, desktop
  environments, language runtimes, developer tools. Contains groups like
  `gnome-apps`, `container-management`, `debugging`, `java-platform`,
  graphical environments.

**26 group IDs are shared** between BaseOS and AppStream (e.g., `core`,
`development`, `gnome-desktop`), but their **package lists are disjoint** —
each repo's comps.xml only lists packages that exist in that repository. The
`core` group in BaseOS contains system packages; the `core` group in AppStream
contains application-layer services (rsyslog, insights-client). When DNF merges
comps across repos, union semantics combine these into one logical group with
all packages from both repos.

**1 environment ID is shared** (`server-product-environment`), with different
mandatory group lists: BaseOS version has 5 mandatory groups, AppStream adds
`container-management` for 6 total.

**Fedora** uses a fundamentally different model: a single unified "Everything"
repository per release (no BaseOS/AppStream split), plus a separate updates
repository. Both repos carry **nearly identical comps.xml files** with the same
groups and environments:

- **Fedora 42 base**: 162 groups, 21 environments
- **Fedora 42 updates**: 163 groups (adds `critical-path-cloud`), 21 environments
- Package lists for shared groups are **identical or near-identical** (e.g.,
  `core`, `base-x`, `gnome-desktop` are byte-for-byte identical; `kde-desktop`
  differs by 2 packages)
- Environments are **completely identical** (same mandatory/optional groups)

This is the opposite of RHEL's partitioned model. Fedora's comps merging is
straightforward: base and updates define the same logical groups with the same
packages, and updates may add new groups or make minor adjustments (package
additions/removals) to existing groups as the release evolves.

Both distributions use the same comps.xml schema — the format originates from
Red Hat's Anaconda installer and has been stable for over 15 years. Fedora comps
files are maintained in [fedora-comps](https://pagure.io/fedora-comps); RHEL
comps are internal to Red Hat.

### Comps merging (dnf/libsolv)

The same group or environment id can appear in multiple repos' comps files.
When this happens, the merge semantics differ by field type.

#### Scalar fields: last-loaded wins

For scalar fields (name, description, display_order), the value from the
**last-loaded repo** wins. Load order is determined by alphabetical sorting
of the `.repo` configuration filenames in `/etc/yum.repos.d/`.

#### Package lists: union semantics

Group package lists are merged as a **union** — packages from all repos'
definitions of the same group are included. If a package appears in both
repos, the behavior depends on the consumer:

- **libcomps** (used by `groups-manager`): the package type from the
  last-loaded repo overrides the earlier one for the same package.
- **DNF5/libsolv**: the package appears under both types (e.g. both
  `mandatory` and `default`). This is a union of types, not a last-wins
  override.

Packages unique to one repo are always preserved in the merged result.

#### Environment group lists: union semantics

Environment `grouplist` (required groups) and `optionlist` (optional groups)
are also merged as a union across repos.

#### Groups and environments have separate namespaces

A group and an environment can share the same `id` without conflict.
`group install same-id` installs only the group, `environment install
same-id` installs only the environment, and `install @same-id` installs
both.

#### Merging with empty package lists

When the same group exists in two repos but one definition has an empty or
missing package list, the merge preserves the package list from the
definition that has one. Updated scalar fields (e.g. description) from the
empty-packagelist definition still take effect.
...
comma/space-separated list of architectures the element applies to.

In theory, when present, the element should only be processed if the target
architecture matches.

In practice, DNF and libsolv do not perform any filtering of comps content
by architecture. All groups, environments, categories, and packages are loaded
regardless of their `arch` attribute values.
